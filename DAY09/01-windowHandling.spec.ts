...
const childPage = await newPage  // "newPage" variable is the reference to resolve the promise for waitForEvent() method 
                                // "childPage" variable is the reference of the page that got invoked by the click action once the waitForEvent() is resolved

/* We will be using await only after waitForEvent() has completed the action of listening and capturing the page and so we will resolve the promise only after the click action */
