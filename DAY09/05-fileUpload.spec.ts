...
2. Without input tag and type = "file" => moderan REACT application */
...
test(`Learn to Upload a file with input tag and type="file`,async ({page}) => {
    ...
    await uploadFile.setInputFiles(path.join(__dirname,`../../Data/Abolute.png`)) // Inject the file into the application usinf setInputFiles()
...
test.only(`Learn to Upload a file without input tag and type="file`,async ({page}) => {
    ...
    await fileUpload.setFiles([path.join(__dirname,`../../Data/Abolute.png`),path.join(__dirname,`../../Data/Feb_28.png`)])
